...
with col1:
    ...
    # Tampilan Koefisien (Menggunakan display text/code agar update real-time)
    c1, c2 = st.columns(2)
    with c1:
        st.caption("b1")
        st.code(f"{b1_lpf:.15f}")
        st.caption("b2")
        st.code(f"{b2_lpf:.15f}")
    with c2:
        st.caption("a0")
        st.code(f"{a0_lpf:.15f}")
        st.caption("a1")
        st.code(f"{a1_lpf:.15f}")
        st.caption("a2")
        st.code(f"{a2_lpf:.15f}")
...
with col2:
    ...
    with c1:
        st.caption("b1")
        st.code(f"{b1_hpf:.15f}")
        st.caption("b2")
        st.code(f"{b2_hpf:.15f}")
    with c2:
        st.caption("a0")
        st.code(f"{a0_hpf:.15f}")
        st.caption("a1")
        st.code(f"{a1_hpf:.15f}")
        st.caption("a2")
        st.code(f"{a2_hpf:.15f}")
...
with col3:
    ...
    if fc_low_bpf >= fc_high_bpf:
        ...
        with c1:
            st.caption("b1")
            st.code(f"{b1_bpf:.15f}")
            st.caption("b2")
            st.code(f"{b2_bpf:.15f}")
        with c2:
            st.caption("a0")
            st.code(f"{a0_bpf:.15f}")
            st.caption("a1")
            st.code(f"{a1_bpf:.15f}")
            st.caption("a2")
...
with col4:
    ...
    if fc_low_bsf >= fc_high_bsf:
        ...
        with c1:
            st.caption("b1")
            st.code(f"{b1_bsf:.15f}")
            st.caption("b2")
            st.code(f"{b2_bsf:.15f}")
        with c2:
            st.caption("a0")
            st.code(f"{a0_bsf:.15f}")
            st.caption("a1")
            st.code(f"{a1_bsf:.15f}")
            st.caption("a2")
